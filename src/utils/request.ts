...
import { getKey, setAuthority } from './authority';
...
request.interceptors.response.use(async (response, options) => {
  ...
  if (response.status === 200) {
    ...
    if(res.status===401) //token失效
    {
      proResponse.status = res.status;
      proResponse.success =false;
      proResponse.msg = res.message;


    //  Modal.info({
    //    content:`您的账号已在其他地方登录或者登录信息已过期!`,
    //    onOk:()=>{
